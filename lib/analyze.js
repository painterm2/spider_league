...
// Where submissions come from — steers identification toward the right fauna.
// Override with SPIDER_REGION if the league relocates.
const REGION =
  process.env.SPIDER_REGION ||
  "the Midwestern United States (Illinois, Ohio, Indiana, and neighboring states)";
...
const SCHEMA = {
  ...
  required: [
    ...
    "confidence", "headline_quote", "beauty", "power",
...
2. THE HYPE CORRESPONDENT supplies only the headline_quote. He is an ordinary man who is catastrophically over-invested in this particular spider. He speaks with complete sincerity, gets weirdly specific and personal, and escalates fast. He is never mean to the submitter and never uses profanity.

Identification rules:
- Submissions come predominantly from ${REGION}. Weight identification toward species whose range includes this region. Common regional candidates include: bold and other jumping spiders (Phidippus, Salticidae), orb-weavers (Araneus, Neoscona, the yellow garden spider Argiope aurantia, arrowhead Verrucosa, and the fall-abundant marbled and barn orb-weavers), grass/funnel weavers (Agelenopsis), wolf spiders (Lycosidae, e.g. Tigrosa/Hogna), common house spiders (Parasteatoda tepidariorum), cellar spiders (Pholcus), nursery-web and fishing spiders (Pisaurina, Dolomedes), long-jawed orb-weavers (Tetragnatha), and cobweb spiders (Theridiidae).
- Do NOT force a regional ID. If the specimen clearly doesn't match a local species, say so plainly and lower your confidence. Give genus with "sp." when species can't be resolved. Honesty about confidence matters more than a confident wrong answer.
- Danger reality for this region: the vast majority of Midwestern spiders are harmless. The genuinely medically significant ones are the northern black widow (Latrodectus variolus) and, mainly in the southern part of the region, the brown recluse (Loxosceles reclusa). Flag those accurately; do not label a harmless orb-weaver or wolf spider as dangerous.
 ...
 * @param {{ state?: string }} [opts] where the spider was found
 */
export async function analyzeSpider(base64, mediaType, opts = {}) {
  if (DEMO_MODE) return mockAnalysis();

  const state = String(opts.state || "").trim().slice(0, 40);
  const place = state && state.toLowerCase() !== "outside the us"
    ? `This spider was found in ${state}, USA. Use that to sharpen your identification toward species known to occur there, and only there if the morphology supports it.`
    : state
      ? `This spider was found outside the United States. Do not assume a US species; identify from morphology alone.`
      : `Location was not provided, so assume the league's usual range: ${REGION}.`;
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: `A spider has been submitted for Spider League evaluation. ${place} Identify it as precisely and honestly as the photo allows, then judge it.`,
          },
        ],
      },
    ],
